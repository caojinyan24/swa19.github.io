...
!function ($) {
  ...
  $(function () {
    ...
    function orderTheLeftNavigations(){
        // 只在正文下做出修改
        if($('.docs-content article').length) {
            var len = $("#markdown-toc").length;
            var toc = $("#markdown-toc").html();
            toc = toc || '';

            $('#navigation .sidenav').html(toc);
            ...
            // 如果没有内容，改变宽度
            if(!len) {
                $('.docs-container .row .col-md-3').hide();
                $('.docs-container .row .col-md-9').removeClass('col-md-9').addClass('col-md-12');
            }
        }
  	}
